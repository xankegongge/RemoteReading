...
namespace DataProvider
{
    public class SqlServerProvider : DataProvider
    {
        ...
        public bool SPExcuteNoneQuery(string spText, SqlParameter []pars, out Dictionary<string, object> outvalues)
       {
        ...
        public bool Login(string id, string pwdMD5)
        {
            bool result = false;
            try
            {
                if (con.State == ConnectionState.Closed)
                    con.Open();
                transaction = con.BeginTransaction();
                com = new SqlCommand("dbo.spgetuserbyuserid", con);
                com.CommandType = CommandType.StoredProcedure;
                com.Transaction = transaction;//添加事务
                com.Parameters.Add("@UserID", SqlDbType.NVarChar).Value = id;

                reader = com.ExecuteReader();

                if (reader.Read())//如果读出了数据
                {
                    string passwd = reader["PasswordMD5"].ToString();
                    int usertype=int.Parse(reader["UserType"].ToString());
                    if (passwd == pwdMD5 && usertype==0)
                    {
                        result = true;
                    }
                }     
