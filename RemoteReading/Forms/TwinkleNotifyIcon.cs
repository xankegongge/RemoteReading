...
namespace RemoteReading
{
    ...
    public interface ITwinkleNotifySupporter
    {
        ...
        Icon GetMedicalReadingIcon();

        void ShowMedicalReading(string gid);
    }
    ...
    public partial class TwinkleNotifyIcon : Component
    {
        ...
        private List<UnhandleMedicalReadingMessageBox> medicalReadingQueue = new List<UnhandleMedicalReadingMessageBox>();
        ...
        public void Initialize(ITwinkleNotifySupporter getter, Control ctrl)
        {
            ...
            this.notifyIcon1.Icon = this.normalIcon;//托盘默认图片
        ...
        void notifyIcon1_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                lock (this.locker)
                {
                    ...
                    if (this.medicalReadingQueue.Count > 0)
                    {
                        UnhandleMedicalReadingMessageBox cache = this.medicalReadingQueue[0];
                        this.medicalReadingQueue.RemoveAt(0);
                        this.DetectUnhandleMessage();//更新闪动图片提示
                        this.twinkleNotifySupporter.ShowMedicalReading(cache.Gid);
                        if (this.UnhandleMessageGone != null)//让列表中对应的GID闪动
                        {
                            this.UnhandleMessageGone(UnhandleMessageType.MedicalReading, cache.Gid);
                        }
                        return;
                    }
        ...
        {
            if (this.friendQueue.Count == 0 && this.groupQueue.Count == 0&&this.medicalReadingQueue.Count==0)
            {
        ...
        public void PushMedicalReadingMessage(string gid, int informationType, byte[] info, object tag)
        {
            lock (this.locker)
            {
                try
                {
                    ...
                    //接下来准备将消息压入queue
                    UnhandleMedicalReadingMessageBox cache = null;
                    lock (this.locker)
                    {
                        //先查看queue中目标gid对应的Cache是否存在，存在直接保存
                        for (int i = 0; i < this.medicalReadingQueue.Count; i++)
                        {
                            if (this.medicalReadingQueue[i].Gid == gid)
                            {
                                cache = this.medicalReadingQueue[i];
                                break;
                            }
                        }
                        ...
                        {
                            cache = new UnhandleMedicalReadingMessageBox(gid);
                            this.medicalReadingQueue.Add(cache);
                            //触发UnhandleMessageOccured事件
                            if (this.UnhandleMessageOccured != null)
                            {
                                this.UnhandleMessageOccured(UnhandleMessageType.MedicalReading, gid);
                            }
                        }
                        if (cache != null)
                        {
                          //  cache.MessageList.Clear();
                   ...
                   // string userName = this.twinkleNotifySupporter.GetFriendName(userID);
                    this.notifyIcon1.Text = string.Format("{0}条阅片消息",  cache.MessageList.Count);
                    //获取阅片头像，将其作为托盘图标
                    this.twinkleIcon = this.twinkleNotifySupporter.GetMedicalReadingIcon();
        ...
        {           
            lock (this.locker)
            {
                try
                {
                    ...
                    lock (this.locker)
                    {
                        //先查看queue中目标好友对应的Cache是否存在，存在直接保存
        ...
        public UnhandleMedicalReadingMessageBox PickoutMedicalReadingMessageCache(string gid)
        {
            lock (this.locker)
            {
                for (int i = 0; i < this.medicalReadingQueue.Count; i++)
                {
                    UnhandleMedicalReadingMessageBox tmp = this.medicalReadingQueue[i];
                    if (tmp.Gid == gid)
                    {
                        this.medicalReadingQueue.RemoveAt(i);
                        this.DetectUnhandleMessage();
                        //if (this.UnhandleMessageGone != null)
                        //{
                        //    this.UnhandleMessageGone(UnhandleMessageType.Friend, userID);
        ...
        private void DetectUnhandleMessage()
        {
            ...
            else if (this.medicalReadingQueue.Count > 0)
            {
                UnhandleMedicalReadingMessageBox cache = this.medicalReadingQueue[0];
    ...
    #endregion
    public class UnhandleMedicalReadingMessageBox
    {
        public UnhandleMedicalReadingMessageBox(string gid)
        {
            this.Gid = gid;
        }
