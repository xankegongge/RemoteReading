...
namespace JustLib.Records
{
    ...
    public class ChatMessageRecord 
    {
        ...
        #region OccureTime
        private string occureTime = DateTime.Now.ToString();
        /// <summary>
        /// 聊天记录发生的时间。
        /// </summary>
        public string OccureTime
        {
    ...
    public class SqliteChatRecordPersister : DefaultChatRecordPersister
    {
        public SqliteChatRecordPersister(string sqlitePath)
        {
            try
            {
                ...
                if (isNew)
                {
                    string sql = "CREATE TABLE ChatMessageRecord (AutoID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, SpeakerID VARCHAR( 0, 20 ) NOT NULL, AudienceID VARCHAR( 0, 20 ) NOT NULL, IsGroupChat BOOLEAN NOT NULL, Content BLOB NOT NULL, OccureTime VARCHAR( 0, 50 ) NOT NULL ); "
