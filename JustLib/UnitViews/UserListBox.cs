...
namespace JustLib.UnitViews
{
    public partial class UserListBox : UserControl
    {
        ...
        public void Initialize(IUser current ,IHeadImageGetter getter ,IUserInformationForm form)
        {
            ...
            this.AssureCatalog(this.currentUser.DefaultFriendCatalog);//添加默认分组我的好友分组至节目
        ...
        public void AddUser(IUser friend)//添加好友至子列表中
        {
            ...
            string strrole = friend.IsExpert ? "      专家" : "";
            
           
            ChatListSubItem subItem = new ChatListSubItem(friend.ID, "", friend.PersonName + strrole, friend.HospitalName, this.ConvertUserStatus(friend.UserStatus), PlatformType.PC, this.resourceGetter.GetHeadImage(friend));
            subItem.Tag = friend;
