...
namespace _CUSTOM_CONTROLS._ChatListBox
{
    ...
    public class ChatListSubItem : IComparable
    {
        ...
        int IComparable.CompareTo(object obj) {
            ...
            DateTime mydate = DateTime.Parse(this.Time); 
            DateTime otherdate = DateTime.Parse(subItem.Time);
            if (this.Status > subItem.Status)
            {
                return 1;

            }
            else if (this.Status < subItem.Status)
            {
                return -1;
            }
            else
        ...
        public ChatListSubItem(string id, string nicname, string displayname, string personalmsg,string time, UserStatus status, Image head)
        {
            this.id = id;
            this.nicName = nicname;
            this.displayName = displayname;
