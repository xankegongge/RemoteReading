...
namespace _CUSTOM_CONTROLS
{
    public partial class ChatListBox : Control
    {
                    ...
                    for (int i = 0, lenI = this.items.Count; i < lenI; i++) {
                        if (items[i].IsOpen) {
                            for (int j = 0, lenJ = items[i].SubItems.Count; j < lenJ; j++) {
                                if (items[i].SubItems[j]!=null&&items[i].SubItems[j].IsTwinkle) {
        ...
        protected virtual void DrawLargeSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem) {
            ...
            if (szFont.Width > 0) {             //判断是否有备注名称
                ...
                g.DrawString(  subItem.NicName ,
                    this.Font, Brushes.Gray, rectSubItem.Height + 150, rectSubItem.Top + 5);
            } else {                            //如果没有备注名称 这直接绘制昵称
                g.DrawString(subItem.NicName, this.Font, Brushes.Black, rectSubItem.Height, rectSubItem.Top + 12);
            }           //绘制个人签名
            g.DrawString(subItem.PersonalMsg, this.Font, Brushes.Blue, rectSubItem.Height, rectSubItem.Top + 12 + this.Font.Height);
            g.DrawString(subItem.Time, this.Font, Brushes.DimGray, rectSubItem.Height + 200, rectSubItem.Top + 12 + this.Font.Height);
        }
        /// <summary>
        /// 绘制小图标模式的个人信息
        /// </summary>
        ...
        protected virtual void DrawSmallSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem) {
            ...
            g.DrawString("\t" + subItem.Time, this.Font, Brushes.Gray, rectSubItem, sf);
        }
